	...
	--[[ Propagate one control's change to the rest of its group.
	     ...
	     Two guards keep this from turning into an O(rooms^2) storm. Writing to a
	     synced control fires that control's own EventHandler, which would
	     propagate the value back out across the whole group again: at 256 rooms
	     one fader move became ~65,000 writes. `syncing` blocks the re-entry, and
	     the value comparison means an already-equal control is never written at
	     all, so no event is raised in the first place. ]]--
	syncing = false
